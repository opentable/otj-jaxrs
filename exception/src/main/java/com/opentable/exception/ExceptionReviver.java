...
class ExceptionReviver implements Function<Map<String, Object>, OTApiException>
{
    ...
    private final Constructor<? extends OTApiException> constructor;

    ExceptionReviver(Class<? extends OTApiException> klass)
    {
    ...
    @Override
    @Nullable
    public OTApiException apply(@Nullable Map<String, Object> input)
    {
        if (input != null && type.equals(input.get(OTApiException.ERROR_TYPE)) &&
                (subtype == null || subtype.contains(input.get(OTApiException.ERROR_SUBTYPE))))
        {
            try {
